...
Пока тексты лежат рядом (`data/inbox`, `data/readers`) **или** он сам ходит в белый список интернета (Википедия, GitHub raw, docs.python.org). «поучи python в интернете» — жуёт страницы в веса. «изучи как делается ЦРМ в битриксе» — другое: сам ищет, коротко записывает, **сам решает, чему учиться дальше** (например PHP, потому что в смарт-процессах Битрикса есть блоки кода). Весь веб не открыт: охота идёт через поиск Википедии, не через сырой HTML academy.
